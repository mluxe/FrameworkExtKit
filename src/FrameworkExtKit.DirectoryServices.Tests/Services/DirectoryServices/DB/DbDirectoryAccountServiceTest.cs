    ...
    public class DbDirectoryAccountServiceTest : DirectoryAccountServiceTest {
        ...
        protected override void test_ldap_record_yufei(DirectoryAccount user) {
            ...
            Assert.Multiple(() => {
                ...
                Assert.AreEqual("LYufei@exchange.company.com", user.Mail);
                ...
                Assert.AreEqual(1, user.Managers.Length);
                Assert.AreEqual("RH6 6NZ", user.PostalCode);
                Assert.AreEqual("Buckingham Gate,  West Sussex", user.Street);
                ...
                Assert.AreEqual("GB0080", user.Geosite);
                ...
                Assert.AreEqual("82154011-Applications Support Analyst", user.JobCode);
                Assert.AreEqual("82154011", user.JobCodeID);
                Assert.AreEqual("Applications Support Analyst", user.JobCodeName);
                Assert.AreEqual("IT Infrastructure", user.BusinessCategory);
                ...
                //Assert.AreEqual(2, user.EDMWorkStations.Length);
                Assert.AreEqual("Data Processing", user.JobCategory);
                Assert.AreEqual("00001540-IT Service Operation and Sustaining", user.JobGroup);
                Assert.AreEqual("00001540", user.JobGroupId);
                Assert.AreEqual("IT Service Operation and Sustaining", user.JobGroupName);
